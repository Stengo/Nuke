...
open class ImageLoader: ImageLoading {
    ...
    fileprivate func dataTaskWith(_ request: ImageRequest, key: ImageRequestKey) -> DataTask {
    ...
    fileprivate func dataTask(_ dataTask: DataTask, didCompleteWithData data: Data?, response: URLResponse?, error: Error?) {
        ...
        if let data = data, let response = response, let cache = conf.cache {
    ...
    fileprivate func decodeData(_ data: Data, response: URLResponse?, task: ImageTask) {
